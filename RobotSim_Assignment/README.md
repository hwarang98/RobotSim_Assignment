...
**사람이 판단·소유한 부분**
- **모든 빌드·컴파일·PIE 실행·테스트 실행과 그 결과 검증**은 사람이 직접 수행했습니다 (AI는 빌드/실행을 하지 않습니다).
...
**작업 방식**: 각 단계를 먼저 프롬프트(스펙)로 정리하고, 사람이 제약을 덧붙여 확정한 뒤 구현했습니다.
커밋 단위·커밋 메시지·범위 결정은 사람이 통제했습니다.
