...
Unreal Engine **5.7** / C++ 중심 프로젝트.
**Unreal 내장 IK 플러그인(IK Rig, FABRIK, Control Rig 등)과 물리 엔진 기반 로봇 구동을 사용하지 않고**,
6-DOF 시리얼 로봇팔의 기구학·동역학·작업 제어를 순수 수학 레이어로 직접 구현했다.
...
**의도적으로 포기한 것**(토크 제어 구동, 시간최적 S-curve, 마찰 원뿔 grasp, N층 적재, Sim2Real 실기):
근거와 트레이드오프는 [`Docs/SCOPE_AND_PRIORITY.md`](Docs/SCOPE_AND_PRIORITY.md)에 정리했다.
과제 안내가 "전부 완성 불필요 — 범위 우선순위 문서화 필수, 판단력도 평가"라 명시하므로, **무엇을 왜
안 했는가**를 그 문서가 답한다.

이 README는 **진입점 겸 요약**이다. 각 단계의 설계 근거·수식 유도·검증은 [`Docs/Steps/`](Docs/Steps/)의
단계별 문서 17편에 있으며, 아래 각 섹션에서 링크한다.

> **개발 방식 고지**: 이 프로젝트는 AI 페어 프로그래밍(Claude Code)으로 진행했다. AI가 관여한 범위와
> 사람이 판단·검증한 범위는 [§10 AI 사용 범위](#10-ai-사용-범위)에 정직하게 기재했다.

---

## 0. 데모 — 멀티로봇 픽앤플레이스

과제의 핵심 시나리오다. 로봇 2대가 팔레트의 박스를 집어 레일에 옮기며, **dispatcher가 도달 가능성을
기준으로 각 로봇에 박스를 배분**한다(작업 할당). 모든 구동은 고정 타임스텝으로 돌아 결정론적이고,
매 스텝의 관절각·각속도·EE 자세·**RNEA 토크**가 CSV로 기록된다.

**실행 절차**:

1. `Content/Maps/` 의 멀티로봇 씬을 연다 (World Settings의 GameMode/HUD가 배선된 맵).
2. **PIE 실행** → dispatcher가 박스를 스폰하고 도달성 캐시를 계산한 뒤 두 로봇에 작업을 배급한다.
3. 화면 UMG 대시보드에서 로봇별 **FSM 단계 · 관절각 · 토크 게이지 · 진행률**, dispatcher의 **미할당
   박스/빈 슬롯/할당 현황**을 실시간으로 본다.
4. 사이클이 끝나면 `<Project>/Saved/RobotSim/` 아래에 로봇별 물리값 CSV와 `Dispatch.csv`(할당 이벤트)가 남는다.
5. **모션 재생**: 대시보드의 재생 컨트롤(D-02)로 저장된 CSV를 읽어 팔 궤적을 그대로 되돌린다 —
   기록↔재생이 대칭이라 "결정론적 재현"이 눈으로 확인된다.
...
> [STEP_D-02](Docs/Steps/STEP_D-02.md) 참고. Step A의 IK 단독 조작은 아래 §1~§3.
...
> 아래 §2~§6은 **Step A(IK)** 의 수학·조작·테스트·설계근거 상세다. Step B~D는 §0 데모와 §7 대응표,
> 단계 문서를 참고하라.
...
총 **40개** Automation Test, 7개 그룹. 모두 `IMPLEMENT_SIMPLE_AUTOMATION_TEST` + `ProductFilter`.
...
> 회귀 무수정 통과**로 삼았다. 마감 시간 배분상의 판단이며 [SCOPE_AND_PRIORITY](Docs/SCOPE_AND_PRIORITY.md)에 근거를 남겼다.
...
> 이 표는 **Step A(IK 단독)** 의 한계다. Step B~D(동역학·픽앤플레이스·멀티로봇·UI)의 한계와 범위 근거는
> [§11 보완 사항](#11-제출-전-점검--알려진-보완-사항) 및 [SCOPE_AND_PRIORITY](Docs/SCOPE_AND_PRIORITY.md)에 있다.
...
UObject에 의존하지 않으며, `static` 함수만 가진 plain struct 패턴으로 통일돼 있다. 픽앤플레이스/dispatcher
액터는 이 수학 레이어를 **소비만** 하고 수정하지 않으므로, STEP A 기구학의 회귀 위험이 구조적으로 0이다.

---

## 10. AI 사용 범위

이 프로젝트는 **AI 페어 프로그래밍(Anthropic Claude Code)** 으로 진행했다. 과제 안내가 "완성도보다
사고 과정과 수학/물리의 정확성이 중요"라 명시하므로, AI가 무엇을 했고 사람이 무엇을 판단·검증했는지
투명하게 밝힌다.

**AI가 관여한 부분**
- 단계별 구현 계획 초안, C++ 코드 작성(수학 레이어·액터·테스트), Doxygen 주석·단계 문서 초안.
...
**사람이 판단·소유한 부분**
- **모든 빌드·컴파일·PIE 실행·테스트 실행과 그 결과 검증**은 사람이 직접 수행했다 (AI는 빌드/실행 안 함).
...
**작업 방식**: 각 단계를 먼저 프롬프트(스펙)로 정리하고, 사람이 제약을 덧붙여 확정한 뒤 구현했다.
커밋 단위·커밋 메시지·범위 결정은 사람이 통제했다.

**검증 책임**: 저장소의 모든 코드는 사람이 빌드·테스트로 검증한 상태로 커밋됐다. AI 산출물을
그대로 신뢰하지 않고, 수학은 골든 테스트로, 거동은 PIE·CSV로 확인하는 것을 원칙으로 삼았다.
...
**민감정보 점검** (제출 전 확인 완료)
- 소스·설정에 API 키·비밀번호·개인 이메일 등 민감정보 없음.
- `Config/DefaultEngine.ini` 의 `SecurityToken` 은 **UE가 UDP 메시징용으로 자동 생성하는 세션 식별자**로,
  외부 인증·과금과 무관하며 보안 위협이 아니다. (신경 쓰이면 삭제해도 에디터가 재생성한다.)
- `Saved/` · `Intermediate/` · `Binaries/` · `DerivedDataCache/` 는 산출물이므로 zip 제출 시 제외 권장
  (GitHub 제출이면 `.gitignore` 로 이미 제외됨).

**정확성·안정성 관점의 알려진 한계** (상세는 각 단계 문서 및 [SCOPE_AND_PRIORITY](Docs/SCOPE_AND_PRIORITY.md))
- **수학 팔(1.6m) ↔ KUKA 메시(3.2m) 크기 불일치**: 시각 그리퍼와 수학 EE가 자세마다 어긋난다. 파지/적재
  지점은 고정점 반복으로 흡수하나 궤적 중간은 그리퍼가 수학 EE를 따르지 않는다. 근본 해결은 `LinkOffsets`
  실측 재작성 + FK/IK 전면 재검증.
- **로봇 링크 충돌체 없음**: 팔끼리·팔과 박스가 통과한다. 배치와 수직 진입으로 회피한다.
- **동역학이 구동에 관여하지 않음**: RNEA 토크는 계산·표시·로깅만 하고 준정적 근사(q̈=0)다 —
  중력·마찰 항은 맞고 관성·코리올리 항은 빠졌다. CSV/게이지의 토크를 그 전제로 읽어야 한다.
- **재생 타임스텝 의존**: 다른 `FixedTimeStepSec` 로 기록된 CSV는 재생 속도가 달라진다.

**여유가 있었다면 다음 순서**: B-03 질량행렬 → B-06 computed torque 제어 → B-07 마찰 원뿔 grasp.
이 셋은 서로 의존해 "셋 다 하거나 셋 다 안 하거나"이므로 이번엔 안 하는 쪽을 택했다
