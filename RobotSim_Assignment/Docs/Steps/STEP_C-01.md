...
CSV는 `<Project>/Saved/RobotSim/PickPlace.csv`에 25개 컬럼으로 기록한다:
...
1. **토크가 없다.** 기구학 구동이며 B-02의 RNEA는 연결되지 않았다. CSV의 물리값은 관절각/각속도와
   EE 자세뿐이고 토크 컬럼이 없다. B-06에서 computed torque 제어가 들어와야 채워진다.
