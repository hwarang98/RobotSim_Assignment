...
- **아직 어디에도 연결되지 않았다.** 액터/런타임 경로가 RNEA를 호출하지 않으므로 실제 시뮬레이션
  ...
  > **D-01에서 해소됨(부분).** `APickPlaceTaskActor`가 고정 스텝마다 `SolveInverseDynamicsRNEA`를,
  > 정지 자세에서는 `ComputeGravityTorque`를 호출해 그 결과를 CSV `tau0_nm`~`tau5_nm` 컬럼과 UMG
  > 토크 게이지로 내보낸다. 두 공개 함수가 모두 데모 경로에 들어왔다.
  > **다만 여전히 구동에는 관여하지 않는다** — 토크는 계산되어 표시될 뿐 원인이 아니고, D-01의
  > 호출은 qdd=0 준정적 근사다. 시뮬레이션 거동이 실제로 바뀌는 것은 B-06 computed torque 제어부터다.
  > 자세한 내용은 `STEP_D-01.md`의 "RNEA를 데모에 연결하는 지점" 참조.
